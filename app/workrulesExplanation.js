(function renderWorkrulesExamples() {
  ...
  setHtml("example-night-overtime", `De nachttoeslag volgt de waarde van het betreffende overuur. Een uur tegen 150% is ${euro.format(hourlyRate * overtimeFactor)}. Bij een nachttarief van ${number.format(nightTotalPercent)}% komt daar ${euro.format(hourlyRate * overtimeFactor * nightExtraFactor)} bij; samen is dat <strong>${euro.format(hourlyRate * overtimeFactor * (1 + nightExtraFactor))}</strong>.`);
