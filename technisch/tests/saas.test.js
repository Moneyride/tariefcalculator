...
test("projectpagina hergebruikt de calculator en projectsservice", async () => {
  ...
  assert.match(html, /id="project-progress-ring"/);
  assert.match(html, /id="project-earned-total"/);
  assert.match(html, /class="account-section project-management-section"/);
  assert.doesNotMatch(html, /id="back-to-projects"/);
  assert.match(script, /pageBackButton\.hidden = showingList/);
  assert.match(script, /newProjectButton\.hidden = !showingList/);
  assert.match(script, /const elapsedDays = current\.days\.filter/);
  assert.doesNotMatch(script, /Tot nu toe verdiend · inclusief vandaag/);
  ...
  assert.match(html, />Tot nu toe verdiend</);
  assert.match(script, /project-earned-total/);
